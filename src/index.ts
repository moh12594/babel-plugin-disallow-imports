...
function shouldThrowError({
  ...
  if (!!disallowedImporters
    && disallowedImporters.some((importer: string) => filename.indexOf(importer) !== -1)) {
      return true
  }
    ...
    visitor: {
      ImportDeclaration(path: any, state: IState) {
        if (!state.opts.properties || !Array.isArray(state.opts.properties)) {
          warn(`No properties array has been provided to ${pluginName}.\nMake sure to provide an array of properties`)
          return
        }
        ...
        state.opts.properties.forEach((property) => {
          ...
          if(!Boolean(disallowedImport)) {
            throw new Error(`No disallowedImport has been provided to ${pluginName}.`)
          }
  
          if (!Boolean(message)) {
            warn(`No custom message were provided.\nPlease add a custom error message for more readability.`)
          }
  
          if (Boolean(disallowedImporters) && Boolean(allowedImporters)) {
            warn(`You provided both disallowedImporters and allowedImporters.\nThe ${pluginName} will take only disallowedImporters.`)
          }
          ...
          if (shouldThrowError({ disallowedImporters, allowedImporters, filename })) {
